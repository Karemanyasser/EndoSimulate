...
using Valve.VR;
public class Splinepolyp : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (isMoving)
        {
            ...
            spline.ElapsedTime=Mathf.Clamp(spline.ElapsedTime,0.09099402f,4.076532f);
           if(ScrollValue[1] > 0.00f && spline.ElapsedTime <4.076532f ){
            spline.ElapsedTime += speed * Time.deltaTime;
        }

        if( ScrollValue[1] <  0.00f && spline.ElapsedTime > 0.09099402f){
            spline.ElapsedTime -= speed * Time.deltaTime;
        }
    ...
    void CheckCollision()
    {
        if (Vector3.Distance(sphere.position, targetObject.position) <= collisionDistance)
        {
            ...
            // Make the first patient 95% transparent
            MakeTransparent(Patient1);

            // Make the second patient 95% transparent
