...
using Valve.VR;
public class Splineabnormal : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (isMoving)
        {
            ...
            spline.ElapsedTime=Mathf.Clamp(spline.ElapsedTime,0.02985145f,4.140822f);
           if(ScrollValue[1] > 0.00f && spline.ElapsedTime <4.140822f ){
            spline.ElapsedTime += speed * Time.deltaTime;
        }

        if( ScrollValue[1] <  0.00f && spline.ElapsedTime > 0.02985145f){
            spline.ElapsedTime -= speed * Time.deltaTime;
        }
