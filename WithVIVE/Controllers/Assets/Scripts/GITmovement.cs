...
public class GITmovement : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        //     // distancePercentage += speed * Time.deltaTime / splineLength;
            ...
            Mathf.Clamp(spline.ElapsedTime,0.01193717f, 1.166262f);
           if(inputreader.forward > 0.008f && spline.ElapsedTime < 1.166262f ){
            spline.ElapsedTime += speed * Time.deltaTime;
        }

        if( inputreader.forward <  -0.008f && spline.ElapsedTime > 0.01193717f){
            spline.ElapsedTime -= speed * Time.deltaTime;
        }
        }else{
            CheckCollision();
